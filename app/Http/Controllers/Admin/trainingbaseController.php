...
class trainingbaseController extends Controller
{
    ...
    public function getradomtopics(Request $request)
    {
        try {
            $this->validate($request, [
                'level'    => 'nullable',
            ...
            $topicRecord = DB::table('topics')
                ...
                ->inRandomOrder()
                ->limit($request->input('quantity'))->get();
            $topicList   = [];
            ...
            return $this->successresponse($topicList);
        } catch (QueryException $e) {
            Log::error('trainingController->getradomtopics->QueryException异常' . $e->getMessage());
            DB::rollBack();
            return $this->failureresponse('数据库查询出错了');
        }
    }
